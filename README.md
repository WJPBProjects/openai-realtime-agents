...
This is a demonstration of more advanced patterns for voice agents, using the OpenAI Realtime API and the OpenAI Agents SDK, adjusted from the [OpenAI Realtime Agents repo](https://github.com/openai/openai-realtime-agents)
...
This is demonstrated in the [chatSupervisor](src/app/agentConfigs/chatSupervisor/index.ts) Agent Config. The chat agent uses the realtime model (`gpt-realtime`) to converse with the user and handle basic tasks, like greeting the user, casual conversation, and collecting information, and a more intelligent, text-based supervisor model (e.g. `gpt-4.1`) is used extensively to handle tool calls and more challenging responses. Speech recognition is powered by `gpt-4o-transcribe`, which converts the user's audio into text before the agents respond. You can control the decision boundary by "opting in" specific tasks to the chat agent as desired.
...
- **Lower cost**: If your chat agent is only being used for basic tasks, the `gpt-realtime` model keeps spend low even when combined with GPT-4.1 for supervision.
...
3. To reduce cost, you can continue to use `gpt-realtime` for the chatAgent and/or `gpt-4.1-mini` for the supervisor model. For speech recognition, this demo relies on `gpt-4o-transcribe`. To maximize intelligence on particularly difficult or high-stakes tasks, consider trading off latency and adding chain-of-thought to your supervisor prompt, or using an additional reasoning model-based supervisor that uses `o4-mini`.
...
_In this simple example, the user is transferred from a greeter agent to a haiku agent. See below for the simple, full configuration of this flow._
